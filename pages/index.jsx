...
// Employee buckets from Hunter headcount field
const SMALL_HEADCOUNTS = ["1-10","11-50","1_10","11_50","1-20","1_20"];
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5,flexWrap:"wrap"}}>
            ...
            {isHot && <span style={{background:"rgba(239,68,68,0.18)",color:"#ef4444",fontSize:9,
              ...
              🔥 HOT LEAD</span>}
            {isKmu && <span style={{background:"rgba(59,130,246,0.12)",color:"#93c5fd",fontSize:9,
              fontFamily:"'IBM Plex Mono',monospace",padding:"2px 6px",borderRadius:2,flexShrink:0}}>
              KMU</span>}
...
export default function Home() {
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      ...
      let current = places.map(p=>({
        ...
        headcount:null,isKmu:false,gfDerived:null,
      }));
      setLeads([...current]);
      ...
      for (let i=0;i<current.length;i++) {
        ...
        // Process Hunter
        let hunterEmails=null,hunterPattern=null,hunterOrg=null,hunterError=null,headcount=null,isKmu=false;
        if (domain && hunterResult.status==="fulfilled" && hunterResult.value) {
          ...
          isKmu = !headcount || SMALL_HEADCOUNTS.some(s=>headcount.startsWith(s.split("_").join("-")));
        ...
        current[i]={...current[i],
          ...
          hunterDone:true,hunterAvgConf,headcount,isKmu,
          hrLoading:false,
  ...
  const secondLeads  = leads.filter(l=>!l.hunterPattern && !l.hunterEmails?.length && l.hunterDone)
    .concat(leadsWithPattern.filter(l=>l.hunterAvgConf<85 && l.hunterAvgConf>0));
  const smallLeads   = leadsWithPattern.filter(l=>l.isKmu);
  const largeLeads   = leadsWithPattern.filter(l=>!l.isKmu && l.headcount);
        ...
        <div style={{maxWidth:960,margin:"0 auto",padding:"22px 28px 48px"}}>
          ...
          {done && activeTab==="small" && (
            <div style={{marginBottom:12,padding:"10px 14px",
              ...
              👥 KMU Klein (1–20 MA, Quelle: Hunter Headcount): Hohe Wahrscheinlichkeit dass Info-E-Mail direkt beim GF landet — persönliche Ansprache ideal.
            </div>
          )}
          {done && activeTab==="large" && (
            <div style={{marginBottom:12,padding:"10px 14px",
              ...
              🏭 KMU Groß (21+ MA): Direkter GF-Kontakt via Handelsregister + Hunter-Muster empfohlen.
            </div>
          )}
